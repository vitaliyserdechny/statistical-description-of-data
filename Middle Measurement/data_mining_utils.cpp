...
double trimmed_mean(int* arr, int size, double percent_ignored) {
    int ignored_count = (percent_ignored * size) / 100;
    int sum = 0;
    for (int i = ignored_count; i < size - ignored_count; i++)
        sum += arr[i];
...
void rand_integer_data(int* arr, int size, int minrnd, int maxrnd) {
    if (arr == nullptr)
        return;
    for (int i = 0; i < size; i++)
